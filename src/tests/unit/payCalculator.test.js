...
await Promise.all([
  ...
  test('calculateShiftPay: normal-rate late deduction pays only for hours worked, no overpay (P1-24)', () => {
    // 4-hr shift at $10/hr, 3 hrs late → worked 1 hr. $10/hr ≥ MIN_WAGE, so the
    // floor does NOT apply: grossPay is exactly 1 hr × $10 = $10 (must NOT inflate
    // to min-wage × scheduled hours — that would pay for unworked late time).
    const result = calculateShiftPay(
      shift('Morning', 'Wednesday', '8:00 AM', '12:00 PM'),
      role('Cook', 10),
      180
    )
    assert.ok(Math.abs(result.grossPay - 10) < 0.01, `expected $10, got ${result.grossPay}`)
    assert.ok(Math.abs(result.lateDeduction - 30) < 0.01, `expected $30, got ${result.lateDeduction}`)
  }),

  test('calculateShiftPay: normal late deduction (not floored) still works (P1-24)', () => {
    // 8-hr shift at $15/hr, 30 min late → effectiveHours=7.5, grossPay=$112.50.
    // $15/hr ≥ MIN_WAGE → no floor. Unchanged from prior behavior.
    const result = calculateShiftPay(
  ...
  test('calculateShiftPay: sub-min-wage rate is lifted to the legal floor for hours worked (P1-24)', () => {
    // 4-hr shift at $5/hr (illegally below MIN_WAGE), 1 hr late → worked 3 hrs.
    // raw = 3 × $5 = $15; min-wage floor for the 3 worked hrs = 3 × $7.25 = $21.75.
    // grossPay floors UP to $21.75 (you can't pay below min wage for time worked).
    const result = calculateShiftPay(
      shift('Short', 'Monday', '9:00 AM', '1:00 PM'),
      role('Trainee', 5),
      60
    )
    assert.ok(Math.abs(result.grossPay - 21.75) < 0.01, `expected $21.75 floor, got ${result.grossPay}`)
  }),
])
